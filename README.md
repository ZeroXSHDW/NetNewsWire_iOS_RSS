...
`make lint` validates manifest invariants and byte-compares every committed OPML, source table and notification matrix against freshly generated artifacts. `make check` runs generation, linting and deterministic tests together.
...
The tool canonicalizes links, removes already processed items, enriches recognized feed names/URLs with manifest section, folder, signal and notification metadata, and marks unmatched sources. It assigns conservative fuzzy duplicate-story groups before sorting by publication time and maintaining a local state file. It truncates oversized article text and caps the default package at 180,000 text characters; adjust `--max-item-chars` or `--max-total-chars` when needed. Feed the resulting `digest-input.json` to [Apple-Intelligence-RSS-Summary-Prompt.md](Apple-Intelligence-RSS-Summary-Prompt.md). Use `--dry-run` to inspect input without advancing state.
